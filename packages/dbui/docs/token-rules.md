...
| **R10** | **Charts draw from the four `viz/*` families and nothing else** — `categorical` for identity, `sequential` for magnitude, `level` for a judgement, `neutral` for absence. Never hand-pick hue primitives for a series, and never borrow `status/*` or `text/*`: those are tuned to contrast with the page, not with a neighbouring mark. | `viz/<hue>/<step>` referenced by chart code instead of a `viz/*` semantic, or a `status/*` semantic used as a mark fill. | ◐ partial (`no-module-color-literal` catches a hue resolved into a chart module, which is the form this took; naming the wrong *semantic* is still unchecked) |
