...
/**
 * A colour literal anywhere in a module, not only inside a className.
 *
 ...
 * and the charts drew the same series colours in light and dark for as long as
 * it stood. A value spelled out in a module has no modes to switch between.
 *
 * Approved or not. This is the one colour rule that does not consult the hex
  ...
  for (const node of sourceFile.getDescendantsOfKind(SyntaxKind.StringLiteral).concat(
    ...
    violations.push({
      ...
      element: node.getParent()?.getKind() === SyntaxKind.PropertyAssignment
        ...
        : "module",
      message: `\`${found[0]}\` is a colour written into a module, so it has one value in both modes.`,
