 ...
 * violations, callers should chain into dbui_lookup_component / dbui_lookup_icon
    ...
    if (hex) {
      if (!APPROVED_HEX.has(hex)) {
        violations.push({
          ...
          fix: `Call dbui_lookup_token({ value: "${hex}", type: "color" }) to get the right token, then use the suggested Tailwind class.`,
          source: cls,
        })
      } else {
        violations.push({
          ...
          fix: `Replace with the named class. Call dbui_lookup_token({ value: "${hex}", type: "color" }) for the token name.`,
          source: cls,
        })
      }
      continue
      ...
      if (SPACING_PREFIXES.has(prefix)) {
        if (!APPROVED_SPACING.has(px)) {
          violations.push({
            ...
            fix: `Use ${nearestSpacing(px)}px → ${suggestSpacingClass(prefix, nearestSpacing(px))}. Call dbui_lookup_token({ value: "${px}px", type: "spacing" }) for details.`,
            source: cls,
          })
        } else {
          violations.push({
    ...
    if (/^#[0-9a-fA-F]{3,8}$/.test(text)) {
      ...
      if (hex && !APPROVED_HEX.has(hex)) {
        violations.push({
          ...
          fix: `Use Tailwind classes or var(--token). Call dbui_lookup_token({ value: "${hex}", type: "color" }).`,
          source: `${name}: ${text}`,
        })
      }
    }
  ...
  for (const imp of sourceFile.getImportDeclarations()) {
    ...
    if (NON_DBUI_ICON_LIBS.has(mod) || NON_DBUI_ICON_LIBS.has(mod.split("/")[0]) || mod.startsWith("@heroicons/") || mod.startsWith("react-icons/")) {
      ...
      violations.push({
        ...
        fix: `For each icon, call dbui_lookup_icon({ query: "<icon name or concept>" }) to find the DBUI equivalent in @muditmittal/dbui/components/icons/.`,
        source: imp.getText(),
      })
    }
  }
  ...
  if (!isComponent) {
    ...
    if (!DBUI_SET.has(tagName)) {
      violations.push({
        ...
        fix: `Verify intentional. If a DBUI replacement exists, call dbui_lookup_component({ query: "<what this does>" }) to find it.`,
        source: opening.getText().slice(0, 100),
      })
    }
  }
