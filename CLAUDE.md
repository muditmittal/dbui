...
**EVERY product page MUST start with the Base shell.** The Base shell (`packages/dbui-shells/src/Base.tsx`) provides:
- Platform Header (48px, bg-muted) — with workspace switcher, search, Genie Code, app switcher, profile menu
- Platform Nav sidebar (180px, collapsible) — with nav items grouped by category
- Content surface (bg-background, rounded-md, border) — where page content goes
- Assistant Panel (360px, toggled by Genie Code icon) — agentic chat side panel

When building any new page or composition:
1. Start with `<Base>` — never build the header/nav/chrome from scratch
2. Build your page content inside the Base's children
3. Base manages sidebar collapse, Genie panel toggle, and responsive behavior
...
In Figma: the Base Shell composition on the Shells page is the starting point for all product screens. Duplicate it, then replace the content area.
...
### COMPOSITION BUILDING — COMPONENT-FIRST RESOLUTION (mandatory):
- IMPORTANT: When building compositions in Figma, ALWAYS resolve every UI element to an existing component instance first. Never create raw frames for controls, menu items, chips, inputs, icons, etc.
- Before writing ANY `use_figma` code for a composition, build a **resolution table** mapping each code element to its Figma component match (✅ Instance) or flagging it (⚠️ Raw frame needed). Present this table before proceeding.
- Raw frames that substitute for missing components must be named with prefix `⚠ Custom:` and reported in the build manifest.
- After building, report: `N linked instances, M flagged raw frames`.
- Full protocol with component selection guide: see `specs/composition-rules.md` → "Figma Composition Building Protocol"
...
**How to find the right icon — MANDATORY before inserting ANY icon in Figma or code:**
1. **ALWAYS look up `icon-index.csv` first** — this is the canonical index (449 icons with Classification, Label, Synonyms)
2. Search by concept: `grep -i "genie" icon-index.csv` → finds `GenieCode,Object,Genie code,"AI/BI, generative, agentic, AI query"`
3. Search by synonym: `grep -i "flask\|lab" icon-index.csv` → finds `Beaker,Object,Experiment,...`
4. If the index doesn't have what you need, THEN grep the icon JSDoc: `grep "use:object" packages/dbui/src/components/icons/`

**NEVER guess an icon name. ALWAYS check `icon-index.csv` first.**
**NEVER use `use:component` icons outside their control. NEVER use `use:action` icons to represent objects.**
