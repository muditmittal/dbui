...
A [dialog](#dialog) is a window overlaid on either the primary window or another dialog window.
...
The [alertdialog](#alertdialog) role is a special-case dialog role designed specifically for dialogs that divert users' attention to a brief, important message.
Its usage is described in the [Alert Dialog Pattern](../alertdialog/alertdialog-pattern.html).
...
The [Alert Dialog Pattern](../alertdialog/alertdialog-pattern.html) is often employed in such circumstances.
...
- The element that serves as the dialog container has a role of [dialog](#dialog).
...
A value set for the [aria-labelledby](#aria-labelledby) property that refers to a visible dialog title.

- A label specified by [aria-label](#aria-label).

- Optionally, the [aria-describedby](#aria-describedby) property is set on the element with the `dialog` role to indicate which element or elements in the dialog contain content that describes the primary purpose or message of the dialog.
