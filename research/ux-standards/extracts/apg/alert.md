...
An [alert](#alert) is an element that displays a brief, important message in a way that attracts the user's attention without interrupting the user's task.
...
The [Alert Dialog Pattern](../alertdialog/alertdialog-pattern.html) is designed for situations where interrupting work flow is necessary.
