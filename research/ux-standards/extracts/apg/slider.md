...
- If the slider has a visible label, it is referenced by [aria-labelledby](#aria-labelledby) on the slider element.
Otherwise, the slider element has a label provided by [aria-label](#aria-label).

- If the slider is vertically oriented, it has [aria-orientation](#aria-orientation) set to `vertical`.
