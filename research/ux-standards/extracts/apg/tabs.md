...
**Tab List** —
A set of tab elements contained in a [tablist](#tablist) element.
...
- When a tab list has its [aria-orientation](#aria-orientation) set to `vertical`:
...
- The element that serves as the container for the set of tabs has role [tablist](#tablist).

- Each element that serves as a tab has role [tab](#tab) and is contained within the element with role `tablist`.

- Each element that contains the content panel for a `tab` has role [tabpanel](#tabpanel).

- If the tab list has a visible label, the element with role `tablist` has [aria-labelledby](#aria-labelledby) set to a value that refers to the labelling element.
Otherwise, the `tablist` element has a label provided by [aria-label](#aria-label).

- Each element with role `tab` has the property [aria-controls](#aria-controls) referring to its associated `tabpanel` element.

- The active `tab` element has the state [aria-selected](#aria-selected) set to `true` and all other `tab` elements have it set to `false`.

- Each element with role `tabpanel` has the property [aria-labelledby](#aria-labelledby) referring to its associated `tab` element.

- If a `tab` element has a popup menu, it has the property [aria-haspopup](#aria-haspopup) set to either `menu` or `true`.

- If the `tablist` element is vertically oriented, it has the property [aria-orientation](#aria-orientation) set to `vertical`.
