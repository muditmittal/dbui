...
It has two elements: a disclosure [button](../button/button-pattern.html) and a section of content whose visibility is controlled by the button.
...
- When the content is visible, the element with role `button` has [aria-expanded](#aria-expanded) set to `true`.
