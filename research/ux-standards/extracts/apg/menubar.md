...
A [menu](#menu) is a widget that offers a list of choices to the user, such as a set of actions or functions.
...
A menu is usually opened, or made visible, by activating a [menu button](../menu-button/menu-button-pattern.html), choosing an item in a menu that opens a sub menu, or by invoking a command, such as `Shift` + `F10` in Windows, that opens a context specific menu.
When a user activates a choice in a menu, the menu usually closes unless the choice opened a submenu.

A menu that is visually persistent is a [menubar](#menubar).
...
[Keyboard Navigation Inside Components](../../practices/keyboard-interface/keyboard-interface-practice.html#kbd_general_within),
...
- If the `menubar` has previously contained focus, focus is optionally set on the `menuitem` that last had focus.
Otherwise, it is set on the first `menuitem` that is not disabled.
...
- A [separator](#separator) in a menu is not focusable or interactive.
...
The element serving as the menu has a role of either [menu](#menu) or [menubar](#menubar).
...
- If activating a [menuitem](#menuitem) opens a submenu, the menuitem is known as a parent menuitem.
A submenu's `menu` element is:
...
- A parent menuitem has [aria-haspopup](#aria-haspopup) set to either `menu` or `true`.

- A parent menuitem has [aria-expanded](#aria-expanded) set to `false` when its child menu is not visible and set to `true` when the child menu is visible.
...
The menu container has `tabindex` set to `-1` or `0` and [aria-activedescendant](#aria-activedescendant) set to the ID of the focused item.

- Each item in the menu has `tabindex` set to `-1`, except in a menubar, where the first item has `tabindex` set to `0`.

- When a [menuitemcheckbox](#menuitemcheckbox) or [menuitemradio](#menuitemradio) is checked, [aria-checked](#aria-checked) is set to `true`.

- When a menu item is disabled, [aria-disabled](#aria-disabled) is set to `true`.

- Items in a menu may be divided into groups by placing an element with a role of [separator](#separator) between groups.
For example, this technique should be used when a menu contains a set of [menuitemradio](#menuitemradio) items.

- All [separators](#separator) should have [aria-orientation](#aria-orientation) consistent with the separator's orientation.

- If a menubar has a visible label, the element with role `menubar` has [aria-labelledby](#aria-labelledby) set to a value that refers to the labelling element.
Otherwise, the menubar element has a label provided by [aria-label](#aria-label).

- If a menubar is vertically oriented, it has [aria-orientation](#aria-orientation) set to `vertical`.
The default value of `aria-orientation` for a menubar is `horizontal`.

- An element with role `menu` either has:

[aria-labelledby](#aria-labelledby) set to a value that refers to the menuitem or button that controls its display.

- A label provided by [aria-label](#aria-label).

- If a menu is horizontally oriented, it has [aria-orientation](#aria-orientation) set to `horizontal`.
The default value of `aria-orientation` for a menu is `vertical`.

### Note

If [aria-owns](#aria-owns) is set on the menu container to include elements that are not DOM children of the container, those elements will appear in the reading order in the sequence they are referenced and after any items that are DOM children.
