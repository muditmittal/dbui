...
A [button](#button) is a widget that enables users to trigger an action or event, such as submitting a form, opening a dialog, canceling an action, or performing a delete operation.
...
To tell assistive technologies that a button is a toggle button, specify a value for the attribute [aria-pressed](#aria-pressed).
...
- Menu button: as described in the [menu button pattern](../menu-button/menu-button-pattern.html), a button is revealed to assistive technologies as a menu button if it has the property [aria-haspopup](#aria-haspopup) set to either `menu` or `true`.

### Note

The types of actions performed by buttons are distinctly different from the function of a link (see [link pattern](../link/link-pattern.html)).
...
- [Button Examples (IDL)](examples/button_idl.html): Examples of clickable HTML `div` and `span` elements made into accessible command and toggle buttons.
This example uses the [IDL Interface](#idl-interface).
...
(see [dialog pattern](../dialog-modal/dialog-modal-pattern.html))
...
- The button has role of [button](#button).
...
However, it can also be provided with [aria-labelledby](#aria-labelledby) or [aria-label](#aria-label).

- If a description of the button's function is present, the button element has [aria-describedby](#aria-describedby) set to the ID of the element containing the description.

- When the action associated with a button is unavailable, the button has [aria-disabled](#aria-disabled) set to `true`.

- If the button is a toggle button, it has an [aria-pressed](#aria-pressed) state.
