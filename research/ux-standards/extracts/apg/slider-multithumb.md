...
A multi-thumb slider implements the [Slider Pattern](../slider/slider-pattern.html) but includes two or more thumbs, often on a single rail.
...
[Slider Pattern](../slider/slider-pattern.html#keyboard_interaction).
...
- Each element serving as a focusable slider thumb has role [slider](#slider).

- Each slider element has the [aria-valuenow](#aria-valuenow) property set to a decimal value representing the current value of the slider.

- Each slider element has the [aria-valuemin](#aria-valuemin) property set to a decimal value representing the minimum allowed value of the slider.

- Each slider element has the [aria-valuemax](#aria-valuemax) property set to a decimal value representing the maximum allowed value of the slider.

- When the range (e.g. minimum and/or maximum value) of another slider is dependent on the current value of a slider, the values of [aria-valuemin](#aria-valuemin) or [aria-valuemax](#aria-valuemax) of the dependent sliders are updated when the value changes.

- If a value of `aria-valuenow` is not user-friendly, e.g., the day of the week is represented by a number, the [aria-valuetext](#aria-valuetext) property is set to a string that makes the slider value understandable, e.g., "Monday".

- If a slider has a visible label, it is referenced by [aria-labelledby](#aria-labelledby) on the slider element.
Otherwise, the slider element has a label provided by [aria-label](#aria-label).

- If a slider is vertically oriented, it has [aria-orientation](#aria-orientation) set to `vertical`.
The default value of `aria-orientation` for a slider is `horizontal`.
