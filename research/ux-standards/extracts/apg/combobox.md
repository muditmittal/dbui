...
A [combobox](#combobox) is an input widget that has an associated popup.
The popup enables users to choose a value for the input from a collection.
The popup may be a [listbox](../listbox/listbox-pattern.html), [grid](../grid/grid-pattern.html), [tree](../treeview/treeview-pattern.html), or [dialog.](../dialog-modal/dialog-modal-pattern.html)
...
- **No autocomplete:** The combobox is editable, and when the popup is triggered, the suggested values it contains are the same regardless of the characters typed in the combobox.
For example, the popup suggests a set of recently entered values, and the suggestions do not change as the user types.

- **List autocomplete with manual selection:** When the popup is triggered, it presents suggested values.
...
- **List autocomplete with automatic selection:** The combobox is editable, and when the popup is triggered, it presents suggested values that complete or logically correspond to the characters typed in the combobox, and the first suggestion is automatically highlighted as selected.
...
Two other widgets that are also visually compact and enable users to make a single choice from a set of discrete choices are [listbox](../listbox/listbox-pattern.html) and [menu button](../menu-button/menu-button-pattern.html).
...
- DOM Focus is maintained on the combobox and the assistive technology focus is moved within the listbox using `aria-activedescendant` as described in
[Managing Focus in Composites Using aria-activedescendant](../../practices/keyboard-interface/keyboard-interface-practice.html#kbd_focus_activedescendant).
...
- DOM Focus is maintained on the combobox and the assistive technology focus is moved within the grid using `aria-activedescendant` as described in
[Managing Focus in Composites Using aria-activedescendant](../../practices/keyboard-interface/keyboard-interface-practice.html#kbd_focus_activedescendant).
...
- DOM Focus is maintained on the combobox and the assistive technology focus is moved within the tree using `aria-activedescendant` as described in
[Managing Focus in Composites Using aria-activedescendant](../../practices/keyboard-interface/keyboard-interface-practice.html#kbd_focus_activedescendant).
...
- If nodes in the tree are arranged horizontally ([aria-orientation](#aria-orientation) is set to `horizontal`):
...
- The dialog implements the keyboard interaction defined in the [modal dialog pattern.](../dialog-modal/dialog-modal-pattern.html)
...
- The element that serves as an input and displays the combobox value has role [combobox](#combobox).

- The combobox element has [aria-controls](#aria-controls) set to a value that refers to the element that serves as the popup.
...
- The popup is an element that has role [listbox](#listbox), [tree](#tree), [grid](#grid), or [dialog](#dialog).

- If the popup has a role other than `listbox`, the element with role `combobox` has [aria-haspopup](#aria-haspopup) set to a value that corresponds to the popup type.
...
- When the combobox popup is not visible, the element with role `combobox` has [aria-expanded](#aria-expanded) set to `false`.
...
- When a descendant of a listbox, grid, or tree popup is focused, DOM focus remains on the combobox and the combobox has [aria-activedescendant](#aria-activedescendant) set to a value that refers to the focused element within the popup.

- For a combobox that controls a listbox, grid, or tree popup, when a suggested value is visually indicated as the currently selected value, the `option`, `gridcell`, `row`, or `treeitem` containing that value has [aria-selected](#aria-selected) set to `true`.
...
Otherwise, if it has a visible label, the combobox element has [aria-labelledby](#aria-labelledby) set to a value that refers to the labelling element.
Otherwise, the combobox element has a label provided by [aria-label](#aria-label).

- The combobox element has [aria-autocomplete](#aria-autocomplete) set to a value that corresponds to its autocomplete behavior:
...
- In ARIA 1.0, the combobox referenced its popup with [aria-owns](#aria-owns) instead of [aria-controls](#aria-controls).
While user agents might support comboboxes with [aria-owns](#aria-owns) for backwards compatibility with legacy content, it is strongly recommended that authors use [aria-controls](#aria-controls).
...
[Listbox Roles, States, and Properties](../listbox/listbox-pattern.html#roles_states_properties)

- [Grid Roles, States, and Properties](../grid/grid-pattern.html#roles_states_properties)

- [Tree Roles, States, and Properties](../treeview/treeview-pattern.html#roles_states_properties)

- [Dialog Roles, States, and Properties](../dialog-modal/dialog-modal-pattern.html#roles_states_properties)
