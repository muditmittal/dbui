...
A [grid](#grid) widget is a container that enables users to navigate the information or interactive elements it contains using directional navigation keys, such as arrow keys, `Home`, and `End`.
...
When presenting content that is tabular, consider the following factors when choosing between implementing this `grid` pattern or the [table](../table/table-pattern.html) pattern.
...
[manages focus movement inside it](../../practices/keyboard-interface/keyboard-interface-practice.html#kbd_general_within).
...
See [Whether to Focus on a Cell or an Element Inside It](#gridNav_focus).
...
If this functionality is needed, see [Editing and Navigating Inside a Cell](#gridNav_inside).
...
If a cell contains an element like a [listbox](../listbox/listbox-pattern.html), then an extra key command to focus and activate the listbox is needed as well as a command for restoring the grid navigation functionality.
Approaches to supporting this need are described in the section on [Editing and Navigating Inside a Cell](#gridNav_inside).
...
See [Key Assignment Conventions for Common Functions](../../practices/keyboard-interface/keyboard-interface-practice.html#kbd_common_conventions)
...
If the cell contains editable content, places focus in an input field, such as a [textbox](#textbox).
If the input is a single-line text field, a subsequent press of `Enter` may either restore grid navigation functions or move focus to an input field in a neighboring cell.
...
- Alphanumeric keys: If the cell contains editable content, places focus in an input field, such as a [textbox](#textbox).

When grid navigation is disabled, conventional changes to navigation behaviors include:

- `Escape`: restores grid navigation.
...
- The grid container has role [grid](#grid).

- Each row container has role [row](#row) and is either a DOM descendant of or owned by the `grid` element or an element with role [rowgroup](#rowgroup).
...
- If there is an element in the user interface that serves as a label for the grid, [aria-labelledby](#aria-labelledby) is set on the grid element with a value that refers to the labelling element.
Otherwise, a label is specified for the grid element using [aria-label](#aria-label).

- If the grid has a caption or description, [aria-describedby](#aria-describedby) is set on the grid element with a value referring to the element containing the description.

- If the grid provides sort functions, [aria-sort](#aria-sort) is set to an appropriate value on the header cell element for the sorted column or row as described in the
...
- If the grid supports selection, when a cell or row is selected, the selected element has [aria-selected](#aria-selected) set `true`.
If the grid supports column selection and a column is selected, all cells in the column have `aria-selected` set to `true`.

- If the grid provides content editing functionality and contains cells that may have edit capabilities disabled in certain conditions, [aria-readonly](#aria-readonly) may be set `true` on cells where editing is disabled.
...
- If the grid includes cells that span multiple rows or multiple columns, and if the `grid` role is NOT applied to an HTML `table` element, then [aria-rowspan](#aria-rowspan) or [aria-colspan](#aria-colspan) is applied as described in the
...
- If the element with the `grid` role is an HTML `table` element, then it is not necessary to use ARIA roles for rows and cells because the HTML elements have implied ARIA semantics.
...
- If rows or cells are included in a grid via [aria-owns](#aria-owns), they will be presented to assistive technologies after the DOM descendants of the `grid` element unless the DOM descendants are also included in the `aria-owns` attribute.
