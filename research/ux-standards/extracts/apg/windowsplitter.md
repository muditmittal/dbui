...
- The separator element has the [aria-valuemin](#aria-valuemin) property set to a decimal value that represents the position where the primary pane has its minimum size.
This is typically `0`.

- The separator element has the [aria-valuemax](#aria-valuemax) property set to a decimal value that represents the position where the primary pane has its maximum size.
This is typically `100`.

- If the primary pane has a visible label, it is referenced by [aria-labelledby](#aria-labelledby) on the separator element.
Otherwise, the separator element has a label provided by [aria-label](#aria-label).
