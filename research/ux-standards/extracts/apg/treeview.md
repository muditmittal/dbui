...
- The `tree` role supports the [aria-activedescendant](#aria-activedescendant) property, which provides an alternative to moving DOM focus among `treeitem` elements when implementing keyboard navigation.
For details, see [Managing Focus in Composites Using aria-activedescendant](../../practices/keyboard-interface/keyboard-interface-practice.html#kbd_focus_activedescendant).
...
- All tree nodes are contained in or owned by an element with role [tree](#tree).

- Each element serving as a tree node has role [treeitem](#treeitem).

- Each root node is contained in the element with role `tree` or referenced by an [aria-owns](#aria-owns) property set on the `tree` element.

- Each parent node contains or owns an element with role [group](#group).

- Each child node is contained in or owned by an element with role [group](#group) that is contained in or owned by the node that serves as the parent of that child.

- Each element with role `treeitem` that serves as a parent node has [aria-expanded](#aria-expanded) set to `false` when the node is in a closed state and set to `true` when the node is in an open state.
...
- If the tree supports selection of more than one node, the element with role `tree` has [aria-multiselectable](#aria-multiselectable) set to `true`.
Otherwise, `aria-multiselectable` is either set to `false` or the default value of `false` is implied.

- The selection state of each selectable node is indicated with either [aria-selected](#aria-selected) or [aria-checked](#aria-checked):
...
- If any nodes are selected, each selected node has either [aria-selected](#aria-selected) or [aria-checked](#aria-checked) set to `true`.
No more than one node is selected at a time if the element with role `tree` does _not_ have [aria-multiselectable](#aria-multiselectable) set to `true`.

- All nodes that are selectable but not selected have either [aria-selected](#aria-selected) or [aria-checked](#aria-checked) set to `false`.

- If the tree contains nodes that are not selectable, neither [aria-selected](#aria-selected) nor [aria-checked](#aria-checked) is present on those nodes.

- Note that except in trees where selection follows focus, the selected state is distinct from focus.
For more details, see [this description of differences between focus and selection](../../practices/keyboard-interface/keyboard-interface-practice.html#kbd_focus_vs_selection)
and [Deciding When to Make Selection Automatically Follow Focus](../../practices/keyboard-interface/keyboard-interface-practice.html#kbd_selection_follows_focus).

- The element with role `tree` has either a visible label referenced by [aria-labelledby](#aria-labelledby) or a value specified for [aria-label](#aria-label).

- If the complete set of available nodes is not present in the DOM due to dynamic loading as the user moves focus in or scrolls the tree, each node has [aria-level](#aria-level), [aria-setsize](#aria-setsize), and [aria-posinset](#aria-posinset) specified.

- If the `tree` element is horizontally oriented, it has [aria-orientation](#aria-orientation) set to `horizontal`.
...
- Conditions that would permit a tree to include both `aria-selected` and `aria-checked` are extremely rare.
...
- If [aria-owns](#aria-owns) is set on the tree container to include elements that are not DOM children of the container, those elements will appear in the reading order in the sequence they are referenced and after any items that are DOM children.
