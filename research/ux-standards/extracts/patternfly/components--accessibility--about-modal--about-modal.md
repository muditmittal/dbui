...
url: https://www.patternfly.org/components/accessibility/about-modal
license: MIT
bucket: A
...
At a minimum, an about modal should meet the following criteria:
    ...
    An <code className="ws-code">aria-label</code> is passed if the about modal does not have a product name heading.</span>} description="This gives the about modal an accessible name, providing context of its purpose or content to users." />


    If there is a brand image, it has a valid <code className="ws-code">alt</code> attribute.</span>} description="If the image is purely decorative then an empty string should be passed as a value so that the image is not exposed to assistive technologies." />
