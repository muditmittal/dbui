...
url: https://www.patternfly.org/foundations-and-styles/styles/motion/motion-development
license: MIT
bucket: A
...
When using the `AnimationsProvider`, you can still pass `hasAnimations` directly to a component to override the global setting of `true` or `false`. For example: `` will override a global setting of `true`.
