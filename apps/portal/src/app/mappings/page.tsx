...
// ─── Color Swatch — reusable across all columns ─────────────────────────────
...
function extractHex(v: string) {
  // Extract just the hex portion, strip parenthetical like "(white)" or "(neutral050)"
...
// Show hex/oklch value by default, strip primitive name annotations.
// e.g. "#F7F7F7 (neutral050)" → "#F7F7F7" but "oklch(0.97 0 0)" stays intact.
function cleanValueForDisplay(v: string) {
  if (!v || v === "—") return null
  // Only strip trailing parenthetical if it follows a hex value — NOT oklch/rgba parentheses
  const cleaned = v.replace(/#([0-9A-Fa-f]{3,8})\s*\([^)]*\)/, "#$1").trim()
  return cleaned
}

// ─── Value Display (below token name) — shows only the active theme mode ────
...
// ─── Sub-value display for shadcn/DuBois columns ────────────────────────────
...
function TokenRow({ token, t, mode }: { token: Token; t: Theme; mode: "light" | "dark" }) {
  ...
  // Build tooltip lines for DBUI column
  ...
  return (
    ...
    >
      ...
      <td
        ...
        style={{ width: "20%", fontFamily: MONO }}
      >
    ...
    >
      ...
      {open && (
        <div className="overflow-x-auto">
          <table
            className="w-full"
            style={{ borderTop: `1px solid ${t.border}`, tableLayout: "fixed" }}
          >
            <colgroup>
              <col style={{ width: "30%" }} />
              <col style={{ width: "25%" }} />
...
// ─── Section stats for header ────────────────────────────────────────────────

const SECTION_STATS = [
  { label: "Semantic", count: SEMANTIC_SURFACE.length + SEMANTIC_TEXT.length + SEMANTIC_ACTION.length + SEMANTIC_BORDER.length + SEMANTIC_CHART.length + SEMANTIC_UTILITY.length + SEMANTIC_BRAND.length },
  { label: "Typography", count: TEXT_STYLES.length + TYPOGRAPHY.length },
  { label: "Numbers", count: RADIUS.length + SPACING.length + SHADOWS.length },
  { label: "Primitives", count: PRIMITIVES.length },
]
    ...
    <div>
      ...
      <section className="pt-24 pb-10 px-8" style={{ borderBottom: `1px solid ${t.border}` }}>
        <div className="max-w-[1100px] mx-auto">
          ...
          <h1 className="text-[36px] leading-[1.1] tracking-[-0.01em] mb-8" style={{ fontFamily: SERIF, color: t.text }}>
            <em className="font-normal">Token Mappings</em>
          </h1>
      ...
      <section className="py-12 px-8">
        <div className="max-w-[1100px] mx-auto space-y-4">
          ...
          {/* Show more — reveals Primitives + Excluded Tokens */}
          {!showPrimitives && (
          ...
          {showExcluded && (
            <>
              ...
              <TokenSection
                title="Excluded from DuBois"
                subGroups={EXCLUDED_DUBOIS_GROUPS}
                query={query}
                t={t}
                mode={mode}
                defaultOpen={false}
              />
              <TokenSection
                title="Excluded from shadcn"
                subGroups={EXCLUDED_SHADCN_GROUPS}
                query={query}
                t={t}
                mode={mode}
                defaultOpen={false}
      ...
      >
        <div className="max-w-[1100px] mx-auto">
          ...
          <div className="grid grid-cols-3 gap-8 text-[13px]">
            <div>
              <div className="mb-1" style={{ color: t.textMuted, fontFamily: MONO }}>
                DBUI Name
              </div>
              <div style={{ color: t.textSubtle }}>
                Figma variable path, e.g.{" "}
                <span style={{ color: t.primary, fontFamily: MONO }}>
