...
export const PANEL_RULES: Rule[] = [
  ...
  {
    id: "N2",
    statement: "One panel per edge.",
    fallback: "One.",
    exception: "None.",
    broken: "A tree rail and a tool rail both docked left, or two sidebars sharing the right edge. Two panels on one edge cannot both be closed from the same place.",
    checked: "screen",
  },
  {
    id: "N3",
    statement: "Left panels open by default. Right and bottom panels start closed.",
    fallback: "The left rail is visible on load and nothing else is.",
    exception: "An editor hides the product nav, because the editor's own tool panel already claims the left edge and the rule above allows one.",
    broken: "A page opens with two panels nobody asked for. Every panel open on load is a decision taken away from the reader.",
    checked: "screen",
  },
  {
    id: "N4",
